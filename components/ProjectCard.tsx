...
import { Project } from "./data/projectsData";// تأكد من المسار
  ...
  return (
    <motion.div
      ...
      className={`relative h-[600px] rounded-[2.5rem] overflow-hidden cursor-pointer transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] bg-[#0B1220] border border-white/5 ${
        isExpanded ? "flex-[4] shadow-2xl shadow-blue-500/10" : "flex-1 hover:border-blue-500/30"
      }`}
    >
      {/* Background Overlay when closed */}
      {!isExpanded && (
        <div className="absolute inset-0 z-10 flex items-center justify-center">
             <motion.div layout className="origin-center rotate-90 whitespace-nowrap text-xl font-bold text-white/20 uppercase tracking-[0.3em]">
                {project.title}
             </motion.div>
        </div>
      )}
      ...
      <div className="flex h-full w-full">
        {/* Left Side: Info (Only visible when expanded) */}
        <AnimatePresence>
          {isExpanded && (
            <motion.div 
            ...
            >
              ...
              <h3 className="text-4xl font-black text-white">{project.title}</h3>
              <p className="text-slate-400 leading-relaxed text-sm">{project.description}</p>
              
              <div className="flex flex-wrap gap-2">
                {project.tags.map(tag => (
                  <span key={tag} className="px-3 py-1 bg-blue-500/10 text-blue-400 text-[10px] rounded-full border border-blue-500/20">
                    {tag}
                  </span>
                ))}
              </div>
        ...
        {/* Right Side: Phone Mockup / Screenshots */}
        <div className={`relative flex-1 flex items-center justify-center p-6 ${!isExpanded ? "opacity-30 grayscale" : "opacity-100"}`}>
           {/* Phone Frame Simulation */}
           <div className="relative w-[280px] h-[560px] bg-black rounded-[3rem] border-[8px] border-slate-800 shadow-2xl overflow-hidden">
                ...
                <div className="h-full w-full overflow-y-auto overflow-x-hidden snap-y snap-mandatory scrollbar-hide">
                    {project.screenshots.map((img, idx) => (
                        <img 
                          key={idx} 
                          src={img} 
                          className="w-full h-full object-cover snap-start border-b-4 border-black" 
                          alt="screenshot" 
                        />
                    ))}
                </div>
